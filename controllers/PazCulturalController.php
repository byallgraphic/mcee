...
use app\models\PazCultura;
use app\models\PazCulturalBuscar;
 ...
 */
class PazCulturalController extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    function actionAgregarCampos()
    {
        ...
        $datatipoPerfil  = PazCultura::find()->where( 'estado=1' )->all();
        $tipoPerfil 	 = ArrayHelper::map( $datatipoPerfil, 'id', 'descripcion' );
        ...
        <div class=row>
            ...
            <div class=cell>
                <?= $form->field($model, '['.$consecutivo.']tipo_perfil')->dropDownList( $tipoPerfil, [ 'prompt' => 'Seleccione...'] ) ?>
            </div>
     ...
     * @return mixed
     */
    public function actionIndex($guardado = 0, $tipo_perfil = '' )
    {
        $idInstitucion = $_SESSION['instituciones'][0];
        if( $idInstitucion > 0 )
        {

            $searchModel = new PazCulturalBuscar();
            $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
            $dataProvider->query
                ->innerJoin( 'tipo_perfil tp', 'tp.id=paz_cultura.tipo_perfil' )
                ->andWhere( 'paz_cultura.estado=1' );
    ...
    {
        $tipo_perfil = '';
        if( Yii::$app->request->get('tipo_perfil') )
            $tipo_perfil = Yii::$app->request->get('tipo_perfil');

        $data = [];
        $idInstitucion = $_SESSION['instituciones'][0];

        if( Yii::$app->request->post('PazCultura') )
            $data = Yii::$app->request->post('PazCultura');
        ...
        $datatipoPerfil  = TipoPerfil::find()->where( 'estado=1' )->all();
        $tipoPerfil 	 = ArrayHelper::map( $datatipoPerfil, 'id', 'descripcion' );

        $dataEstados  = Estados::find()->where( 'id=1' )->all();
        $estados 	  = ArrayHelper::map( $dataEstados, 'id', 'descripcion' );

        if (PazCultura::loadMultiple($models, Yii::$app->request->post() )) {
            ...
            return $this->redirect(['index', 'guardado' => true, 'tipo_perfil' => $tipo_perfil ]);
        }
    ...
    public function actionUpdate($id)
    {
        ...
        $datatipoPerfil  = tipoPerfils::find()->where( 'estado=1' )->andWhere( "categoria='Instancia institucionales'")->all();
        $tipoPerfil 	 = ArrayHelper::map( $datatipoPerfil, 'id', 'descripcion' );
    ...
    public function actionDelete($id)
    {
        ...
        $tipoPerfil  = TipoPerfil::findOne($model->tipo_perfil);

        return $this->redirect(['index', 'guardado'=> 0]);
    }
    ...
    {
        if (($model = PazCultura::findOne($id)) !== null) {
            return $model;
        }
