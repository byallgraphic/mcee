...
use app\models\CbacSeguimientoProceso;
use app\models\CbacSeguimientoProcesoBuscar;
...
use app\models\CbacSemanaLogros;
...
/**
 * CbacSeguimientoProcesoController implements the CRUD actions for CbacSeguimientoProceso model.
 */
class CbacSeguimientoProcesoController extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    /**
     * Lists all CbacSeguimientoProceso models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new CbacSeguimientoProcesoBuscar();
    ...
    /**
     * Displays a single CbacSeguimientoProceso model.
     * @param string $id
     * @return mixed
    ...
    /**
     * Creates a new CbacSeguimientoProceso model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate()
    {
        $model = new CbacSeguimientoProceso();
	
        if ($model->load( Yii::$app->request->post() ) && $model->save(false)) 
		{
			
			$actividadesModel[1] = new CbacPorcentajesActividades();
			$actividadesModel[2] = new CbacPorcentajesActividades();
			$actividadesModel[4] = new CbacPorcentajesActividades();


			if (CbacPorcentajesActividades::loadMultiple($actividadesModel, Yii::$app->request->post())) 
			{
			...
			foreach (Yii::$app->request->post()['CbacOrientacionMetodologicaActividades'] as $key =>  $modelos)
			{
				$OriMetodologica[$key] = new CbacOrientacionMetodologicaActividades();
			}
			
			if (CbacOrientacionMetodologicaActividades::loadMultiple($OriMetodologica, Yii::$app->request->post())) 
			{
			...
			//se crean los modelos con los ids iguales a como vienen del $_POST
			foreach (Yii::$app->request->post()['CbacOrientacionMetodologicaVariaciones'] as $key =>  $modelos)
			{
				$OriMetodologicaOriMetodologicaVariaciones[$key] = new CbacOrientacionMetodologicaVariaciones();
			}
			
			if (CbacOrientacionMetodologicaVariaciones::loadMultiple($OriMetodologicaOriMetodologicaVariaciones, Yii::$app->request->post())) 
			{
    ...
    /**
     * Updates an existing CbacSeguimientoProceso model.
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save() ) 
		{
			...
			if($postPorcentajesActividades =  Yii::$app->request->post()['CbacPorcentajesActividades'])
			{
				$porcentajeActividades = CbacPorcentajesActividades::find()->indexBy('id')->andWhere("id_seguimiento_proceso = $id")->all();
			...
			if($postOrientcacionMetodologica  =   Yii::$app->request->post()['CbacOrientacionMetodologicaActividades'])
			{
				$orientacionMetodologica = CbacOrientacionMetodologicaActividades::find()->indexBy('id')->andWhere("id_seguimiento_proceso = $id")->all();
			...
			//se guardan los datos de la tabla CbacOrientacionMetodologicaVariaciones
			if($postCbacOrientacionMetodologicaVariaciones  =   Yii::$app->request->post()['CbacOrientacionMetodologicaVariaciones'])
			{
				$orientacionMetodologicaV = CbacOrientacionMetodologicaVariaciones::find()->indexBy('id')->andWhere("id_seguimiento_proceso = $id")->all();
				
				foreach($orientacionMetodologicaV as $key => $ori)
				{
					$ori->descripcion = $postCbacOrientacionMetodologicaVariaciones[$ori->id_variaciones_actividades]['descripcion'];	
					$ori->save(false); 	
				}
			}
    ...
    /**
     * Deletes an existing CbacSeguimientoProceso model.
    ...
    /**
     * Finds the CbacSeguimientoProceso model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param string $id
     * @return CbacSeguimientoProceso the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    ...
    {
        if (($model = CbacSeguimientoProceso::findOne($id)) !== null) {
            return $model;
        }
