...
$porcetaje_actividades		= 0;
	...
	{
		// $rom = CbacReporteOperativoMisional::findOne([
							// 'estado'			=> 1,
							// 'id_institucion' 	=> $idInstitucion,
							// 'id_sedes' 			=> $id_sede,
						// ]);
						
		$roms = CbacActividadesRom::find()
		...
		foreach( $roms as $rom )
		{
			...
			foreach( $proyectos as $idProyecto => $descripcionProyecto )
			{
				...
				foreach( $procesos as $idProceso => $descripcionProceso )
				{
					...
					$actividades_rom_upts = CbacActividadesRom::find()
					...
					foreach( $actividades_rom_upts as $actividades_rom_upt )
					{
						...
						$eq 						= CbacEquiposCampo::findOne( $dataActividadesParticipadas->id_equipo_campos );
						if( $eq )
							$equipos[] 				= $eq;
					...
					$modelIntegrante 			= CbacActividadesRomXIntegranteGrupo::find()
																->where( 'estado=1' )
					...
					$poblaciones = CbacTipoCantidadPoblacionRom::find()
										->where('estado=1')
