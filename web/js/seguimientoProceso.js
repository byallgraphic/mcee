...
$('#isaseguimientoproceso-fecha').on('change', function() {
	...
	if( valorFecha != "" )
	{
		...
		$.get( "index.php?r=isa-seguimiento-proceso/datos-avances&fecha="+valorFecha,
			function( data )
			{
				...
				for ( i = 1 ; i<=4; i++)
				{
					$('#isaporcentajesactividades-'+i+'-total_sesiones').val("");
					$('#isaporcentajesactividades-'+i+'-avance_sede').val("");
					$('#isaporcentajesactividades-'+i+'-avance_ieo').val("");
					
					$('#isaporcentajesactividades-'+i+'-total_sesiones').val(datos[0]);
					$('#isaporcentajesactividades-'+i+'-avance_sede').val(datos[1]+"%");
					$('#isaporcentajesactividades-'+i+'-avance_ieo').val(datos[2]+"%");
				}
